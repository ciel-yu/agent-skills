...
`Intake → Extract Semantic Contract → Design Replacement Boundary ──[ADR Gate]──▶ Implement New Path → Verify Semantic Equivalence → Cut Over`
...
- **Design Replacement Boundary** - choose the seam: new module, wrapper, adapter, feature flag, façade, or parallel path. Prefer clean replacement over edits inside legacy internals.
...
1. **During `Extract Semantic Contract`:** [references/semantic-contract.md](./references/semantic-contract.md)
2. **During `Design Replacement Boundary` and cutover planning:** [references/workflow.md](./references/workflow.md)
3. **Only if `ADR Gate` opens:** [references/adr-gate.md](./references/adr-gate.md)
...
- tests or other evidence for semantic equivalence
...
- [ ] Semantic equivalence was checked with tests, fixtures, examples, or other concrete evidence.
